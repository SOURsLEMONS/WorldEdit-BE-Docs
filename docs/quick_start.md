...
1. 放置一块石头，对着石头使用 [「填充方块选取器」](usage/kit) 然后使用 [「填充选区」](usage/kit).创造了一个石头立方(选区填充了石头)
...
3. 使用 [「创世神设置」](usage/kit) 工具. 快捷栏中应该会出现一个菜单 ，选择其中的「笔刷设置」. 然后选择要绑定到的铲子。使其成为半径为 3 的球形画笔和圆石填充。并确认设置退出菜单。
